...
class GraphClient:
    ...
    def get_uniswap_pool_lastly_added(self, last_days=14, min_volume=100, min_liquidity=0, min_tx=100):
        days = int((datetime.datetime.now() - datetime.timedelta(days=last_days)).timestamp())
        q = """
            {
        ...
        """ % (days, min_volume, min_liquidity, min_tx)
        data = self.run_query(self.UNI, q)
        df = pd.json_normalize(data['pairs'])
        df['createdAtTimestamp'] = df['createdAtTimestamp'].apply(lambda x: created_date(int(x)))
        df['pair'] = df['token0.symbol'] + '/' + df['token1.symbol']
        return df[['createdAtTimestamp', 'pair', 'token0.name', 'token1.name', 'volumeUSD', 'txCount', 'totalSupply',
                   'reserveUSD']]
    ...
    def get_uniswap_pools_by_volume(self):
        ...
        return df[['token0.name', 'token0.symbol', 'token1.name', 'token1.symbol', 'volumeUSD',
                   'txCount', ]]
